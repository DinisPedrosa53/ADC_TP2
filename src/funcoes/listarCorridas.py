...
def listar_corridas(ficheiro="corridas.json"):
    try:
        ...
        if corridas:
            print(f"{'Nome':<20} {'pódio':<15} {'país 1':<15} {'cidade':<15} {'tempo':<15} {'data':<15} {'hórario':<15}")
            print("="*95)

            # Listando as corridas
            for corrida in corridas:
                print(f"{corrida['nome']:<20} {corrida['podio']:<15} {corrida['pais']:<15} {corrida['cidade']:<15} {corrida['tempo']:<15} {corrida['data']:<15} {corrida['horario']:<15}")
        else:
